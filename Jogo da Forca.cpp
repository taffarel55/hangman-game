...
char letra_usuario,ACERTOU,POSICAO_PALAVRA=contP,LETRA_REPETIDA,menuW,PALAVRA_ANALISE[15][14];
...
int movimentoquadrado_menu()
{
	...
	//DIREITA	
	if ((opcao=='D')or(opcao=='d')or(opcao==DIREITA)) 
	{	
		if (escolha!=3)		
	    {
	    NE=32;
	...
	//ESQUERDA
	else if ((opcao=='A')or(opcao=='a')or(opcao==ESQUERDA))
	{
     if (escolha!=1)		
	    {
	    NE=32;
